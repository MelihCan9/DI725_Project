...
# def validate(model, val_loader, processor, device, max_length=64, save_examples_path=None):
#     model.eval()
#     all_refs = []
#     all_hyps = []
#     examples = []
...
#                 if len(examples) < 5:  # İlk 5 örneği kaydet
#                     examples.append({
...
#     wandb.init(
#         project="paligemma-phase2-benchmark",  # <== YENİ PROJE ADI
#         name=run_name,
#         config=config
#     )
...
#         val_metrics = validate(model, val_loader, processor, DEVICE, max_length=MAX_LEN, save_examples_path=os.path.join(OUTPUT_DIR, f"val_examples_epoch{epoch+1}.json"))
#         wandb.log({
